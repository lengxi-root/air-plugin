...
export class update extends plugin {
  ...
  async runUpdate(isForce) {
    ...
    if (isForce) {
      command = `git -C ${dirPath}/ checkout . && ${command} && pnpm yarn`;
      this.e.reply("正在执行强制更新操作，请稍等");
    } else {
      this.e.reply("正在执行更新操作，请稍等");
    }
    /** 获取上次提交的commitId，用于获取日志时判断新增的更新日志 */
    this.oldCommitId = await this.getcommitId(basename);
    uping = true;
    let ret = await this.execSync(command);
    uping = false;

    if (ret.error) {
      logger.mark(`${this.e.logFnc} 更新失败：air-plugin`);
      this.gitErr(ret.error, ret.stdout);
      return false;
    }

    /** 获取插件提交的最新时间 */
    let time = await this.getTime(basename);

    if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
      await this.reply(`air-plugin已经是最新版本\n最后更新时间：${time}`);
    } else {
  ...
  async getLog(plugin = "") {
    ...
    let end = "";
      ...
      "更多详细信息，请前往gitee查看\nhttps://github.com/snowtafir/xianxin-plugin/commits/main\n原版请查看：\nhttps://gitee.com/xianxincoder/xianxin-plugin/commits/master";

    log = await this.makeForwardMsg(`air-plugin更新日志，共${line}条`, log, end);
