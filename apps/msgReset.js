...
export default class msgReset extends plugin {
  ...
  async accept(e) {
    ...
    let btnbot = _cfg.button?.btn_users[0];
    ...
    let msgServer = !isQQbot && (mg?.includes(String(group_id))) && (ub == this.e.self_id) && _cfg.msgServer?.open;
  ...
  for (let i of Array.isArray(msg) ? msg : [msg]) {
    ...
    switch (i.type) {
      case 'text':
        ...
        let ht = await fetch("https://v1.hitokoto.cn/?encode=text&min_length=8&max_length=20");
        ht = await ht.text();
        if (typeof _cfg.Ark_set?.Text != 'undefined') {
          i.text = _cfg.Ark_set.Text.replace(/\[时间\]/g, time)
                                    .replace(/\[一言\]/g, ht)
                                    .replace(/\[换行\]/g, '\n')
      ...
      case 'image':
        ...
        ht = await ht.text();
        let wx = _cfg.Ark_set?.img_wx || _cfg.Ark_set?.Text_wx;
        let bt = _cfg.Ark_set?.img_bt ? _cfg.Ark_set.img_bt.replace(/\[时间\]/g, time).replace(/\[一言\]/g, ht) : '';
        let xbt = _cfg.Ark_set?.img_xbt ? _cfg.Ark_set.img_xbt.replace(/\[时间\]/g, time).replace(/\[一言\]/g, ht) : '';
  ...
  for (let i of Array.isArray(msg) ? msg : [msg]) {
    ...
    switch (i.type) {
      case 'text':
        let now = new Date();
        let time = now.toLocaleString();
        let ht = await (await fetch("https://v1.hitokoto.cn/?encode=text&min_length=8&max_length=20")).text();
        if (typeof _cfg.markdown?.Text != 'undefined') {
          i.text = _cfg.markdown.Text.replace(/\[时间\]/g, time)
                                     .replace(/\[一言\]/g, ht)
                                     .replace(/\[换行\]/g, '\n')
                                     .replace(/\[消息内容\]/g, i.text);
        }
...
async function makeMd(msg) {
  let _cfg = await cfg.getConfig('air', 'config');
  let mix_open = _cfg.markdown?.mix_open && typeof _cfg.markdown?.mix_id != 'undefined';
  let isbtn = _cfg.button?.open && (_cfg.button?.template != null || _cfg.button?.template != '');
  let msgs = [];
  let params = [];
  let btn = [];
    ...
    switch (i.type) {
      case 'text':
        if (!mix_open) {
          ...
          msgs.push(segment.markdown({ custom_template_id: _cfg.markdown?.text_id, params }));
      ...
      case 'keyboard':
      case 'button':
        btn.push(segment.raw(i));
        break;
    }
  }
  msgs.push(segment.markdown({ custom_template_id: _cfg.markdown?.mix_id, params }));
  if (btn.length > 0) msgs.push(...btn);
  else if (isbtn) msgs.push(segment.raw({ 'type': 'keyboard', 'id': _cfg.button?.template }));
  return msgs;
}
